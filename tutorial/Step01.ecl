...
Layout_Person := DD.Layout_Person;
Layout_Accounts := DD.Layout_Accounts;
...
//write files to disk
O1 := OUTPUT(PROJECT(base_people_dist,Layout_Person),,'~VISUALIZER::EXAMPLEDATA::People',OVERWRITE);
O2 := OUTPUT(base_kids_dist,,'~VISUALIZER::EXAMPLEDATA::Accounts',OVERWRITE);
O3 := OUTPUT(base_combined,,'~VISUALIZER::EXAMPLEDATA::PeopleAccts',OVERWRITE);

P1 := PARALLEL(O1,O2,O3);

D1 := DATASET('~VISUALIZER::EXAMPLEDATA::People',{Layout_Person,UNSIGNED8 RecPos{VIRTUAL(fileposition)}}, THOR);
D2 := DATASET('~VISUALIZER::EXAMPLEDATA::Accounts',{Layout_Accounts_Link,UNSIGNED8 RecPos{VIRTUAL(fileposition)}}, THOR);
D3 := DATASET('~VISUALIZER::EXAMPLEDATA::PeopleAccts',{Layout_Combined,UNSIGNED8 RecPos{VIRTUAL(fileposition)}},THOR);

I1 := INDEX(D1,{PersonID,RecPos},'~VISUALIZER::EXAMPLEDATA::KEYS::People.PersonID');
I2 := INDEX(D2,{PersonID,RecPos},'~VISUALIZER::EXAMPLEDATA::KEYS::Accounts.PersonID');
I3 := INDEX(D3,{PersonID,RecPos},'~VISUALIZER::EXAMPLEDATA::KEYS::PeopleAccts.PersonID');

I4 := INDEX(D1,{LastName,FirstName,RecPos},'~VISUALIZER::EXAMPLEDATA::KEYS::People.LastName.FirstName');
I5 := INDEX(D1,{PersonID},{FirstName,LastName,MiddleInitial,Gender,Street,City,State,Zip,RecPos},'~VISUALIZER::EXAMPLEDATA::KEYS::People.PersonID.Payload');
I6 := INDEX(D2,{PersonID},{Account,OpenDate,IndustryCode,AcctType,AcctRate,Code1,Code2,HighCredit,Balance,RecPos},'~VISUALIZER::EXAMPLEDATA::KEYS::Accounts.PersonID.Payload');
I7 := INDEX(D1,{LastName,FirstName},{D1},'~VISUALIZER::EXAMPLEDATA::KEYS::People.LastName.FirstName.Payload');
I8 := INDEX(D1,{State,City,Zip,Lastname,Firstname},{D1},'~VISUALIZER::EXAMPLEDATA::KEYS::People.State.City.Zip.LastName.FirstName.Payload');
...
OUTPUT(XMLdata,,'~VISUALIZER::EXAMPLEDATA::XML_timezones',CSV(HEADING('<?xml version=1.0 ...?>\n<timezones>\n','</timezones>\n')),OVERWRITE);
...
OUTPUT(UCCDS,,'~VISUALIZER::EXAMPLEDATA::XML_UCC',CSV(HEADING('<UCC>\n','</UCC>\n')),OVERWRITE);
